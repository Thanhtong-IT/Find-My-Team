...
class _TeamScreenState extends State<TeamScreen> {
  ...
  final List<_JoinRequest> _requests = [
    _JoinRequest(name: 'Alex Storm', game: 'Valorant', rank: 'Immortal', trust: 98),
    _JoinRequest(name: 'Minh Gấu', game: 'Liên Quân Mobile', rank: 'Cao thủ', trust: 95),
  ];
  ...
  void _acceptRequest(int index) {
    if (_members.length >= _teamSize) {
      _showSnackBar('Nhóm đã đủ thành viên', isError: true);
      return;
    }
    final req = _requests[index];
    setState(() {
      _members.add(_Member(name: req.name, role: 'Thành viên', isReady: false, hasMic: true, isOnline: true));
      _requests.removeAt(index);
    });
    _showSnackBar('Đã chấp nhận thành viên');
  }
  ...
  void _createTeam(String game, String rank, int size) {
    setState(() {
      _hasTeam = true;
      _createdGame = game;
      _createdRank = rank;
      _teamSize = size;
      _members.clear();
      _members.add(_Member(name: 'Bạn', role: 'Chủ nhóm', isReady: true, hasMic: true, isOnline: true));
    });
    _showSnackBar('Đã tạo nhóm và public tuyển thành viên');
  }
  ...
  void _disbandTeam() {
    ...
    _showSnackBar('Đã giải tán nhóm');
  }
              ...
              child: IndexedStack(
                ...
                children: [
                  ...
                  _TeamTab(
                    ...
                    members: _members,
                    teamSize: _teamSize,
                    createdGame: _createdGame,
                    createdRank: _createdRank,
                    isSmallScreen: isSmallScreen,
                    onCreateTeam: _createTeam,
                    onDisbandTeam: _disbandTeam,
                    onLeaveTeam: () => _showSnackBar('Chủ nhóm cần giải tán nhóm để rời', isError: true),
                  ),
  ...
  Widget? _buildFab() {
    ...
    if (_selectedTabIndex == 1 && !_hasTeam) return null;
    if (_selectedTabIndex == 1 && _hasTeam) {
      ...
      child: Column(
        ...
        children: [
          ...
          if (requests.isEmpty)
            ...
            ...requests.asMap().entries.map((entry) => Padding(
              padding: const EdgeInsets.only(bottom: 12),
              child: _JoinRequestCard(request: entry.value, isSmallScreen: isSmallScreen, onAccept: () => onAccept(entry.key), onReject: () => onReject(entry.key)),
            )),
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          CircleAvatar(radius: isSmallScreen ? 22 : 26, backgroundColor: AppColors.primary.withValues(alpha: 0.1), child: Icon(Icons.person, color: AppColors.primary, size: isSmallScreen ? 22 : 26)),
          const SizedBox(width: 12),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(request.name, style: TextStyle(fontSize: isSmallScreen ? 14 : 16, fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
                const SizedBox(height: 4),
                Row(
                  children: [
                    _MiniChip(label: request.game, icon: Icons.sports_esports_rounded, isSmallScreen: isSmallScreen),
                    const SizedBox(width: 6),
                    _MiniChip(label: request.rank, icon: Icons.emoji_events_rounded, isSmallScreen: isSmallScreen),
                  ],
                ),
                const SizedBox(height: 8),
                Row(
                  children: [
                    Text('Độ uy tín: ', style: TextStyle(fontSize: isSmallScreen ? 11 : 12, color: AppColors.textSecondary)),
                    Text('${request.trust}%', style: TextStyle(fontSize: isSmallScreen ? 11 : 12, fontWeight: FontWeight.w600, color: AppColors.success)),
                    const SizedBox(width: 8),
                    Expanded(
                      child: ClipRRect(
                        borderRadius: BorderRadius.circular(4),
                        child: LinearProgressIndicator(
                          value: request.trust / 100,
...
class _MiniChip extends StatelessWidget {
  ...
  final IconData icon;
  final bool isSmallScreen;
  const _MiniChip({required this.label, required this.icon, required this.isSmallScreen});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        ...
        children: [
          Icon(icon, size: isSmallScreen ? 10 : 12, color: AppColors.textSecondary),
...
class _TeamTab extends StatelessWidget {
  ...
  final bool isSmallScreen;
  final void Function(String game, String rank, int size) onCreateTeam;
      ...
      child: Column(
        ...
        children: [
          ...
          ...members.asMap().entries.map((entry) {
            ...
            return Padding(padding: const EdgeInsets.only(bottom: 8), child: _MemberRow(member: member, isLeader: idx == 0, isSmallScreen: isSmallScreen));
          }),
                  ...
                  child: ElevatedButton(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        const SizedBox(width: 6),
                        Text('Sẵn sàng', style: TextStyle(fontWeight: FontWeight.bold, fontSize: isSmallScreen ? 13 : 14)),
                      ],
                    ),
                  ),
                ),
...
class _WaitingRoomHeader extends StatelessWidget {
  ...
  const _WaitingRoomHeader({required this.members, required this.teamSize, required this.createdGame, required this.createdRank, required this.isSmallScreen});
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    if (createdGame != null && createdRank != null)
                      Text('$createdGame \u2022 $createdRank', style: TextStyle(fontSize: isSmallScreen ? 11 : 12, color: AppColors.textSecondary)),
                  ],
                ),
              ),
              Container(
    ...
    return Container(
      ...
      child: Row(
        children: [
          Stack(
            children: [
              CircleAvatar(
                ...
                child: Icon(Icons.person, color: AppColors.primary, size: avatarSize / 2),
              ),
              if (member.isOnline)
                Positioned(
                  right: 0,
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    Text(member.name, style: TextStyle(fontSize: isSmallScreen ? 14 : 15, fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
                    if (isLeader) ...[
                      const SizedBox(width: 6),
                      Container(
                ...
                Text(
                  '${member.role} \u2022 ${member.isReady ? "Sẵn sàng" : "Chưa sẵn sàng"}',
          ...
          if (member.hasMic)
            Icon(Icons.mic_rounded, size: isSmallScreen ? 18 : 20, color: AppColors.success)
          else
            Icon(Icons.mic_off_rounded, size: isSmallScreen ? 18 : 20, color: AppColors.textLight),
        ],
      ),
    );
  }
...
class _CreateTeamForm extends StatefulWidget {
  final bool isSmallScreen;
  final void Function(String game, String rank, int size) onSubmit;
...
class _CreateTeamFormState extends State<_CreateTeamForm> {
  ...
  final List<String> _games = ['Liên Quân Mobile', 'Valorant', 'PUBG Mobile', 'Liên Minh Huyền Thoại'];
  final List<String> _ranks = ['Không yêu cầu', 'Đồng', 'Bạc', 'Vàng', 'Bạch kim', 'Kim cương', 'Cao thủ'];
  ...
  void _handleSubmit() {
    if (_selectedGame == null) { _showSnackBar('Vui lòng chọn tựa game', isError: true); return; }
    if (_selectedRank == null) { _showSnackBar('Vui lòng chọn rank yêu cầu', isError: true); return; }
    if (_roleController.text.trim().isEmpty) { _showSnackBar('Vui lòng nhập vị trí cần tuyển', isError: true); return; }
    if (_descController.text.trim().isEmpty) { _showSnackBar('Vui lòng nhập mô tả nhóm', isError: true); return; }
    widget.onSubmit(_selectedGame!, _selectedRank!, _selectedSize);
  }
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 16),
        _buildDropdown(label: 'Tựa game', hint: 'Chọn tựa game', value: _selectedGame, items: _games, icon: Icons.sports_esports_rounded, onChanged: (v) => setState(() => _selectedGame = v)),
        const SizedBox(height: 14),
        _buildDropdown(label: 'Rank yêu cầu', hint: 'Chọn rank', value: _selectedRank, items: _ranks, icon: Icons.emoji_events_rounded, onChanged: (v) => setState(() => _selectedRank = v)),
        const SizedBox(height: 14),
        _buildSizeSelector(),
        const SizedBox(height: 14),
        _buildTextField(label: 'Vị trí / vai trò cần tuyển', hint: 'Ví dụ: Rừng, Trợ thủ...', controller: _roleController, maxLines: 1),
        const SizedBox(height: 14),
  ...
  Widget _buildDropdown({required String label, required String hint, required String? value, required List<String> items, required IconData icon, required ValueChanged<String?> onChanged}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
...
class _JoinRequest {
  ...
  _JoinRequest({required this.name, required this.game, required this.rank, required this.trust});
}
...
class _Member {
  ...
  _Member({required this.name, required this.role, required this.isReady, required this.hasMic, required this.isOnline});
}
