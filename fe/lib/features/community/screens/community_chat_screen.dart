...
class _CommunityChatScreenState extends State<CommunityChatScreen> {
  ...
  void _loadMessages() {
    final msgs = CommunityRepository().getMessages(_currentChannel.id);
  ...
  void _sendMessage() {
    ...
    CommunityRepository().addMessage(_currentChannel.id, msg);
  ...
  Widget _buildMessageList(bool isSmallScreen) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final showDate = index == 0 || !_isSameDay(msg.timestamp, _messages[index - 1].timestamp);
        return Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            if (showDate) _buildDateDivider(msg.timestamp, isSmallScreen),
            _MessageBubble(message: msg, isSmallScreen: isSmallScreen),
          ],
        );
      },
    );
...
class _MessageBubble extends StatelessWidget {
  ...
  const _MessageBubble({required this.message, required this.isSmallScreen});
