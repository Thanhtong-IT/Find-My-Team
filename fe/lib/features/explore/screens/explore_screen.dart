...
class _ExploreScreenState extends State<ExploreScreen> {
  ...
  List<OnlinePlayerModel> get _filteredPlayers {
    return _onlinePlayers.where((player) {
      final matchGame =
          _selectedGame == 'Tất cả game' || player.gameName == _selectedGame;
      final matchSearch =
          _searchQuery.isEmpty ||
          player.displayName.toLowerCase().contains(
            _searchQuery.toLowerCase(),
          ) ||
          (player.gameName?.toLowerCase().contains(
                _searchQuery.toLowerCase(),
              ) ??
              false) ||
          (player.username?.toLowerCase().contains(
                _searchQuery.toLowerCase(),
              ) ??
              false);
  ...
  void _listenTeamEvents() {
    ...
    _exploreTeamReloadSub = AppEventBus.instance.exploreTeamReloadStream.listen(
      (_) {
  ...
  Future<void> _loadInitialData() async {
    ...
    try {
      ...
      await Future.wait([_loadTeams(), _loadOnlinePlayers()]);
    } catch (e, stack) {
  ...
  Future<void> _loadTeams() async {
    ...
    try {
      ...
      if (_selectedGameModel == null) {
        ...
        loadedTeams = await TeamApiService().getOpenTeams(
          gameId: _selectedGameModel!.id,
        );
      }
      if (!mounted) return;
      setState(() {
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              try {
                ...
                await TeamApiService().sendJoinRequest(
                  teamId,
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: BlocBuilder<TeamBloc, TeamState>(
          buildWhen: (prev, curr) =>
              prev.currentTeam?.id != curr.currentTeam?.id,
                  ...
                  children: [
                    ...
                    if (!_isSearchMode) ...[
                              ...
                              onSelected: (value) {
                                setState(() {
                                  ...
                                  if (value == 'Tất cả game') {
                                    ...
                                    _selectedGameModel = _games.firstWhere(
                                      (g) => g.name == value,
                                    );
                                  }
                                });
                      ...
                      (_isLoadingTeams
                          ...
                          ? _EmptyState(
                              message: 'Không có đội nào phù hợp',
                              isSmallScreen: isSmallScreen,
                            )
                          : Column(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              _InfoChip(
                label: team.requiredRank ?? 'Không yêu cầu',
              ...
              _InfoChip(
                label:
                    '${team.currentMemberCount ?? team.members.length}/${team.maxMembers}',
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    if (player.rank != null && player.role != null)
